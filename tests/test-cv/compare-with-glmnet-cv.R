...
#   #proc.time() - pt
#   
#   cv.rat <- cv.gn$cvm/cv.bl$cve
#   return (cv.rat)
# })
# proc.time() - pt
# 
# mn <- apply(sim, 1, mean)
# se <- apply(sim, 1, sd)/sqrt(nsims)
# qtls <- apply(sim, 1, quantile, c(0.05, 0.5, 0.95))
# 
# plot(NA, xlim = range(lambda), ylim = range(sim), log = 'x',
#      xlab = expression(lambda), ylab = "CV Error Ratio")
# grid(); abline(h = 1, lwd = 1.5, col = 'gray50')
# for (i in 1:ncol(sim)) {
#   lines(sim[,i] ~ lambda, col = rgb(0, 0, 0, 0.25))
# }
# lines(mn ~ lambda, lwd = 2, col = 'red')
# lines(mn + se ~ lambda, lwd = 2)
# lines(mn - se ~ lambda, lwd = 2)
# # lines(qtls[1,] ~ lambda, lwd = 2, lty = 'dotted')
# # lines(qtls[2,] ~ lambda, lwd = 2, col = 'red', lty = 'dotted')
# # lines(qtls[3,] ~ lambda, lwd = 2, lty = 'dotted')
