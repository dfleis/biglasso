...
no <- 1000
nx <- 20
dat <- coxed::sim.survdata(N = no, xvars = nx, beta = rnorm(nx, 0, 0.1)*rbinom(nx, 1, 0.5))
...
alpha <- 0.5
penalty <- "enet" 
lambda <- 0.1#exp(seq(-1, -6, length.out = 20))
...
Dmat
round(calc.devs(X, y, beta.bl), 4)
round(calc.devs(X, y, beta.gn), 4)
